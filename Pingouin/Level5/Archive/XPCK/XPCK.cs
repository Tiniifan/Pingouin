    ...
    public class XPCK : IArchive
    {
        ...
        public void Save(string fileName, ProgressBar progressBar = null)
        {
            using (FileStream stream = new FileStream(fileName, FileMode.Create, FileAccess.Write))
            {
                ...
                if (paddingSize != 4)
                {
                    byte[] paddedData = new byte[tableNameData.Length + paddingSize];

                    Array.Copy(tableNameData, paddedData, tableNameData.Length);

                    for (int i = tableNameData.Length; i < paddedData.Length; i++)
                    {
                        paddedData[i] = 0x00;
                    }

                    tableNameData = paddedData;
                }
                ...
                // Write header
                var header = Header;
                (int hight, int low) = XPCKSupport.FileCountToHex(Directory.Files.Count);
